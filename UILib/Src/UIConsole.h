...
public:
   ...
   ~CConsole()
   {
    ...
    void Printf(LPCTSTR szFormat, ...)
	{
	   ...
	   ::WriteConsole(m_hConsole, szMsg, ::lstrlen(szMsg), &dwWritten, NULL); 
	};
   HANDLE Detach()
   {
   ...
   {
      _ASSERTE(wColor>=0x0 && wColor<=0xF);                   // FOREGROUND_BLUE etc
      return SetTextAttribute(wColor);
   }
   BOOL SetBkColor(WORD wColor)
   {
      _ASSERTE(wColor==0 || (wColor>=0x10 && wColor<=0xF0));  // BACKGROUND_BLUE etc
      return SetTextAttribute(wColor);
   }
